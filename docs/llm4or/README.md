## Updated on 2026.09.23
> Usage instructions: [here](./docs/README.md#usage)

<details>
  <summary>Table of Contents</summary>
  <ol>
    <li><a href=#automatic-algorithm-design-/-hyper-heuristics>Automatic Algorithm Design / Hyper-heuristics</a></li>
    <li><a href=#llm-auto-formulation>LLM Auto-Formulation</a></li>
    <li><a href=#l2o-/-learning-to-optimize>L2O / Learning to Optimize</a></li>
    <li><a href=#vrp-/-combinatorial-solving>VRP / Combinatorial Solving</a></li>
    <li><a href=#solver-intelligence>Solver Intelligence</a></li>
  </ol>
</details>

## Automatic Algorithm Design / Hyper-heuristics

|Publish Date|Title|Authors|PDF|Code|
|---|---|---|---|---|
|**2026-09-10**|**T-GADE: Thermodynamical Generative-AI-Driven Evolution of LLM Artifacts**|Kyoko Ogawa et.al.|[2609.12286](http://arxiv.org/abs/2609.12286)|null|
|**2026-09-02**|**RideSkill: A Hierarchical Algorithm for Generalized Ride Sharing with LLM-Driven Automatic Evolution**|Zijian Zhao et.al.|[2609.02250](http://arxiv.org/abs/2609.02250)|null|
|**2026-08-26**|**ES-AHD: An Evolution Strategy Framework for Automatic Heuristic Design**|Yutao Lai et.al.|[2609.00023](http://arxiv.org/abs/2609.00023)|null|
|**2026-08-21**|**Agentic ESOpt: Fine-Tuning Long-Horizon LLM Agents with Minimal GPU Requirements**|Zhi Zheng et.al.|[2608.17310](http://arxiv.org/abs/2608.17310)|null|
|**2026-08-12**|**$\varepsilon$ -MemEvo: Adaptive Cross-Task Memory Transfer for LLM Program Evolution**|Aofan Liu et.al.|[2608.12522](http://arxiv.org/abs/2608.12522)|null|
|**2026-08-07**|**Recipes for Creativity: Iterative Generation and Evaluation in Large Language Models**|Rens Anderson et.al.|[2608.07243](http://arxiv.org/abs/2608.07243)|null|
|**2026-08-04**|**AutoSND: From Execution Evidence to Structural Policies for Automated Network Dismantling Heuristic Discovery**|Zhijing Hu et.al.|[2608.03653](http://arxiv.org/abs/2608.03653)|null|
|**2026-07-30**|**LLM-Guided Evolutionary Search for Constraint Model Reformulation to Improve Solver Efficiency**|Kostis Michailidis et.al.|[2607.28268](http://arxiv.org/abs/2607.28268)|null|
|**2026-07-29**|**FunL2O: LLM-Guided Feature Function Design for Learning to Optimize**|Bingheng Li et.al.|[2607.27389](http://arxiv.org/abs/2607.27389)|null|
|**2026-07-13**|**RefineEvo: Planning-Guided Heuristic Evolution with Bidirectional Experience**|Yang Wu et.al.|[2607.11358](http://arxiv.org/abs/2607.11358)|null|

<p align=right>(<a href=#updated-on-20260923>back to top</a>)</p>

## LLM Auto-Formulation

|Publish Date|Title|Authors|PDF|Code|
|---|---|---|---|---|
|**2026-09-22**|**Optimal Operation Method for Computing Power-Electric Power Coordination Considering End-to-End Completion Latency of Computing Tasks**|Yize Liu et.al.|[2609.25594](http://arxiv.org/abs/2609.25594)|null|
|**2026-09-20**|**STEVE: Stabilizing Textual Gradient-Based Prompt Optimization via Error-Driven Refinement and Regularized Verification**|Yifan Xu et.al.|[2609.23716](http://arxiv.org/abs/2609.23716)|null|
|**2026-09-20**|**PhysAI-Bench: A Benchmark for LLM-Based Agentic Decision-Making in Autonomous UAV-Centric Physical AI**|Mohamed Amine Ferrag et.al.|[2609.23695](http://arxiv.org/abs/2609.23695)|null|
|**2026-09-19**|**A General Multicriteria Optimization Perspective on Resilience**|Stephan Helfrich et.al.|[2609.23188](http://arxiv.org/abs/2609.23188)|null|
|**2026-09-19**|**OptiSkill: A Hierarchical and Evolving SkillBank for LLM-Based Optimization Modeling**|Ruiqing Zhao et.al.|[2609.22987](http://arxiv.org/abs/2609.22987)|null|
|**2026-09-19**|**Real-Validated UAV Audition Under Rotor Ego-Noise for Low-False-Alarm Human Detection**|Junhao Wei et.al.|[2609.22830](http://arxiv.org/abs/2609.22830)|null|
|**2026-09-19**|**SelfOp: An Optimization Algorithm for Self-Improving Security Agents**|Saad Ullah et.al.|[2609.22792](http://arxiv.org/abs/2609.22792)|null|
|**2026-09-18**|**Defusing Explosive Prompts: Understanding and Preventing Trigger-Based Prompt Injections in LLM Agents**|Justin Szczepaniak et.al.|[2609.22510](http://arxiv.org/abs/2609.22510)|null|
|**2026-09-16**|**From Corridor Selection to Earthwork: A Multi-Stage Framework for Automated Road Design via Steiner Trees and Convex Optimization**|Paavai Manimaran Vanjeenathammal et.al.|[2609.19350](http://arxiv.org/abs/2609.19350)|null|
|**2026-09-16**|**Evidence-Grounded Agentic Formulation Development in an Autonomous Laboratory**|Michael M. Craig et.al.|[2609.19099](http://arxiv.org/abs/2609.19099)|null|

<p align=right>(<a href=#updated-on-20260923>back to top</a>)</p>

## L2O / Learning to Optimize

|Publish Date|Title|Authors|PDF|Code|
|---|---|---|---|---|
|**2026-09-22**|**Bespoke: Generating MOOC-Quality Industry-Personalized Lecture Videos at Scale**|Romain Puech et.al.|[2609.26540](http://arxiv.org/abs/2609.26540)|null|
|**2026-09-22**|**Manipulation of Deformable Linear Objects Using Model Predictive Path Integral Control with Bidirectional Long Short-Term Memory Learning**|Lukas Zeh et.al.|[2609.26238](http://arxiv.org/abs/2609.26238)|null|
|**2026-09-22**|**Sometimes You Gotta Run Before You Can Walk: Run-then-Walk Scheduling Strategy for VLM Autonomous Driving**|Yuqi Ye et.al.|[2609.25831](http://arxiv.org/abs/2609.25831)|null|
|**2026-09-21**|**Scalable Dynamic Pricing of Substitutable Products through Structure-Guided Policy Learning**|Yue Su et.al.|[2609.24605](http://arxiv.org/abs/2609.24605)|null|
|**2026-09-21**|**Adversarially Robust PAC Learning with Optimal VC Rates**|Steve Hanneke et.al.|[2609.24260](http://arxiv.org/abs/2609.24260)|null|
|**2026-09-21**|**Reinforcement Learning Inspired Black-box Adversarial Attacks for Computer Vision**|Florian Krone et.al.|[2609.24249](http://arxiv.org/abs/2609.24249)|null|
|**2026-09-18**|**Learning-to-Optimize as the Missing Architectural Layer of AI-Native Networks**|Giambattista Amati et.al.|[2609.21519](http://arxiv.org/abs/2609.21519)|null|
|**2026-09-18**|**Decision-Focused Learning for Mean-Variance Portfolio Optimization via KKT-Based Reformulation**|Kensei Nosaka et.al.|[2609.21427](http://arxiv.org/abs/2609.21427)|null|
|**2026-09-17**|**MATCH: Model-Aware Tool Learning with Curriculum Scheduling and Hierarchically Gated Rewards**|Shihao Liu et.al.|[2609.20082](http://arxiv.org/abs/2609.20082)|null|
|**2026-09-16**|**STUART: Sequence Triage and qUAntification of Read Transcripts for Rapid Ionizing Radiation Exposure Assessment**|Tomasz Strzoda et.al.|[2609.19139](http://arxiv.org/abs/2609.19139)|null|

<p align=right>(<a href=#updated-on-20260923>back to top</a>)</p>

## VRP / Combinatorial Solving

|Publish Date|Title|Authors|PDF|Code|
|---|---|---|---|---|
|**2026-09-22**|**When Should Dependency Updates Invoke Repair Agents? A Lightweight Routing Study**|Liheng Fan et.al.|[2609.25911](http://arxiv.org/abs/2609.25911)|null|
|**2026-09-21**|**New lower bounds for CDS and $f$ -routing**|Atsuya Hasegawa et.al.|[2609.24291](http://arxiv.org/abs/2609.24291)|null|
|**2026-09-21**|**DCL-GPGLS: Dynamic Curriculum Learning for Genetic Programming Guided Local Search in Large-Scale Vehicle Routing**|Saining Liu et.al.|[2609.24105](http://arxiv.org/abs/2609.24105)|null|
|**2026-09-21**|**Genetic Programming with Behaviour-based Niching for Learning Guided Local Search in Vehicle Routing Problems**|Saining Liu et.al.|[2609.24104](http://arxiv.org/abs/2609.24104)|null|
|**2026-09-18**|**Combinatorial Optimization Augmented Machine Learning for Dynamic Electric Autonomous Dial-a-Ride Problem**|Yue Su et.al.|[2609.22495](http://arxiv.org/abs/2609.22495)|null|
|**2026-09-22**|**Recasting the Destroy Step of Large Neighborhood Search as Dense Segmentation**|Yang Liu et.al.|[2609.22350](http://arxiv.org/abs/2609.22350)|null|
|**2026-09-18**|**MA-LIPP: Cooperative Multi-Agent Load-Aware Informative Path Planning for Heterogeneous Robot Teams**|Hojune Kim et.al.|[2609.21167](http://arxiv.org/abs/2609.21167)|null|
|**2026-09-17**|**Vehicle-Routing Clustering by Sparse Quantum Relaxation**|Farzan Moosavi et.al.|[2609.19575](http://arxiv.org/abs/2609.19575)|null|
|**2026-09-16**|**APGEM: Adaptive Policy-Guided Error Mitigation for Quantum Reinforcement Learning on a Real-World CVRP Case Study**|Shabir Ahmad Sofi et.al.|[2609.18219](http://arxiv.org/abs/2609.18219)|null|
|**2026-09-15**|**Data-Driven Stress Testing of Intermodal Freight Networks Using GAN-Generated Disruption Scenarios**|Xudong Wang et.al.|[2609.17373](http://arxiv.org/abs/2609.17373)|null|

<p align=right>(<a href=#updated-on-20260923>back to top</a>)</p>

## Solver Intelligence

|Publish Date|Title|Authors|PDF|Code|
|---|---|---|---|---|
|**2026-09-22**|**Neutral-Atom-based Quantum Optimization for Resource Allocation in NOMA Networks**|Patatchona Keyela et.al.|[2609.26556](http://arxiv.org/abs/2609.26556)|null|
|**2026-09-20**|**Contextual Distributionally Robust Chance-Constrained Programs: Exact Reformulations and Valid Inequalities**|Hanbin Yang et.al.|[2609.23739](http://arxiv.org/abs/2609.23739)|null|
|**2026-09-20**|**A Combinatorial Benders Decomposition Framework for Two-Dimensional Irregular Bin Packing Problems with Convex Polygons**|Jianming Wang et.al.|[2609.23461](http://arxiv.org/abs/2609.23461)|null|
|**2026-09-19**|**Presolve heuristics in HiGHS: implementation and computational study**|Simon Spoorendonk et.al.|[2609.22938](http://arxiv.org/abs/2609.22938)|null|
|**2026-09-14**|**Toward Mission-Critical ISAC: Reliable Energy-Aware Coordination in UAV Swarms**|Masoud Shokrnezhad et.al.|[2609.22292](http://arxiv.org/abs/2609.22292)|null|
|**2026-09-16**|**From Corridor Selection to Earthwork: A Multi-Stage Framework for Automated Road Design via Steiner Trees and Convex Optimization**|Paavai Manimaran Vanjeenathammal et.al.|[2609.19350](http://arxiv.org/abs/2609.19350)|null|
|**2026-09-16**|**Probabilistic Linear Explanations**|Frederic Koriche et.al.|[2609.19077](http://arxiv.org/abs/2609.19077)|null|
|**2026-09-16**|**M $^3$ P-R1: Reinforcement Learning for Large Language Model Guided Multi-Modal Motion Planning via MIP Code Generation**|Xingpeng Sun et.al.|[2609.18669](http://arxiv.org/abs/2609.18669)|null|
|**2026-09-09**|**Economic Evaluation of V2G-Enabled Fast Charging Stations Under Endogenous EV Adoption Dynamics**|Mingjian Tuo et.al.|[2609.10388](http://arxiv.org/abs/2609.10388)|null|
|**2026-09-07**|**A New Perspective on Clustering: A Mixed-norm Model and its Solution by Progressive Integer Programming**|Junyi Liu et.al.|[2609.08002](http://arxiv.org/abs/2609.08002)|null|

<p align=right>(<a href=#updated-on-20260923>back to top</a>)</p>

